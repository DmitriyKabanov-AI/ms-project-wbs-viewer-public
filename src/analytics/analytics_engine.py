        ...
        for folder in sorted(SNAPSHOTS_DIR.iterdir()):
            if folder.is_dir() and not folder.name.startswith("."):
                ...
                if cnt > 0:
                    snaps.append({
                        "id": folder.name,
                        "label": folder.name + " (" + str(cnt) + " files)",
                        "date": folder.name,
                        "xml_count": cnt,
                    })
    return snaps
...
def load_snapshot(snap_id: str) -> Dict[str, Any]:
    ...
    if not xml_dir.exists():
        return {"error": "Folder not found: " + str(xml_dir)}
    xml_files = sorted(xml_dir.glob("*.xml"))
    if not xml_files:
        return {"error": "No XML files in " + str(xml_dir)}
    ...
    for fpath in xml_files:
        try:
            ...
            log.error("Error parsing %s: %s", fpath.name, e)
    return {"projects": projects, "all_tasks": all_tasks}

# ------------------------------------------------------------
#  PV (Planned Value) — linear interpolation
# ------------------------------------------------------------
...
# ------------------------------------------------------------
#  PROJECT METRICS (with Work, forecast, RAG)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
#  RESOURCE METRICS (with Work and load)
# ------------------------------------------------------------
def compute_resource_metrics(all_tasks: List[Task], cutoff: date) -> List[Dict]:
    ...
    for t in all_tasks:
        ...
        for res in (t.resources or ["—no resource—"]):
...
# ------------------------------------------------------------
#  HEATMAP (by Work instead of task count)
# ------------------------------------------------------------
...
def compute_resource_work_profile(all_tasks: List[Task], start_date: date, end_date: date) -> Dict[str, Any]:
    ...
    for task in all_tasks:
        ...
        for res in (task.resources or ["—no resource—"]):
